...
git ls-remote git@github.com:codecentric/$PROJECT_NAME.git >/dev/null 2>&1
if [ $? -eq 0 ]; then
  echo "Warning! There is an existing GitHub repository at git@github.com:codecentric/$PROJECT_NAME.git."
  ...
  echo "Check the codecentric organisation on GitHub and use a unique project name next time."
...
if [ $? -ne 0 ]; then
  ...
  curl -sS -X POST -d '{"name": "'"$PROJECT_NAME"'", "private": false, "team_id": "2049986"}' -H "Authorization: token $GITHUB_TOKEN" -i https://api.github.com/orgs/codecentric/repos >/dev/null
  if [ $? -ne 0 ]; then
...
curl -sS -X POST "https://circleci.com/api/v1.1/project/github/codecentric/$PROJECT_NAME/follow?circle-token=$CIRCLE_TOKEN" >/dev/null
...
curl -sS -X POST --header "Content-Type: application/json" -d '{"hostname":"github.com", "private_key":"'"$PRIVATE_KEY"'"}' "https://circleci.com/api/v1.1/project/github/codecentric/$PROJECT_NAME/ssh-key?circle-token=$CIRCLE_TOKEN" >/dev/null
echo "Adding SSH key to GitHub"
curl -sS -X POST -d '{"key": "'"$PUBLIC_KEY"'", "title": "CircleCI write access", "read_only": false }' -H "Authorization: token $GITHUB_TOKEN" -i https://api.github.com/repos/codecentric/$PROJECT_NAME/keys >/dev/null
...
git remote add origin git@github.com:codecentric/$PROJECT_NAME.git >/dev/null
...
echo "[![CircleCI](https://circleci.com/gh/codecentric/$PROJECT_NAME.svg?style=svg)](https://circleci.com/gh/codecentric/$PROJECT_NAME)" >> README.md
